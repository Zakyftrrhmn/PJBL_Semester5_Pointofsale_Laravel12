...
@section('subtitle', 'Kelola merek produk Anda')
@section('content')
    ...
    <div class="space-y-6" x-data="{ search: '' }">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
            ...
            <div class="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-5">
                ...
                @can('merek.create')
                    ...
                    <a href="{{ route('merek.create') }}"
                        ...
                        <i class='bx bx-plus-circle'></i> Tambah Merek
                    </a>
                @endcan
            </div>
            ...
            <div class="p-5 border-t border-gray-100 sm:p-6">
                        ...
                        <table class="min-w-full text-sm">
                            <thead class="bg-gray-50">
                                <tr class="border-b border-gray-100">
                                    ...
                                    <th class="px-5 py-3 text-left font-medium">Nama Merek</th>
                                    @canany(['merek.edit', 'merek.destroy'])
                                ...
                                @forelse ($mereks as $merek)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-5 py-4 text-gray-700">
                                            {{ $loop->iteration + ($mereks->firstItem() - 1) }}
                                        </td>
                                        <td class="px-5 py-4 text-gray-700">{{ $merek->nama_merek }}</td>
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-2 justify-center">
                                                ...
                                                @can('merek.destroy')
                                                    <!-- Hapus -->
                                                    <button
                                                        @click="showModal = true; deleteUrl = '{{ route('merek.destroy', $merek->id) }}'"
                                                        class="relative inline-flex items-center justify-center rounded-lg p-2 text-xs shadow-sm text-gray-700 border border-gray-200">
                ...
                <!-- Pagination -->
                <div class="mt-4">
                    {{ $mereks->links('vendor.pagination.tailwind') }}
                </div>
            </div>
        </div>
    </div>
