...
@section('subtitle', 'Manage your category')
@section('content')
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($categories as $category)
                                    <tr class="hover:bg-gray-50 transition"
                                        x-show="search === '' || '{{ strtolower($category->category_name) }}'.includes(search.toLowerCase())">
                                        <td class="px-5 py-4 text-gray-700">
                                            {{ $loop->iteration + ($categories->firstItem() - 1) }}
                                        </td>
                                        <td class="px-5 py-4 text-gray-700">{{ $category->category_name }}</td>
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-2 justify-center">
                                                <!-- Edit -->
                                                <a href="{{ route('category.edit', $category->id) }}"
                                                    class="inline-flex items-center justify-center rounded-lg p-2 text-xs shadow-sm text-gray-700 border border-gray-200">
                                                    <i class="bx bx-edit text-base"></i>
                                                </a>

                                                <!-- Delete -->
                                                <button
                                                    @click="showModal = true; deleteUrl = '{{ route('category.destroy', $category->id) }}'"
                                                    class="inline-flex items-center justify-center rounded-lg p-2 text-xs shadow-sm text-gray-700 border border-gray-200">
